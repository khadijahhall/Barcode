...
class AirportRouter extends Router{
...
//@Override
//public static Location findByZip(String zipCode) {
//return findClosestAirport(zipCode);
//  }
  public void determineRoute(Barcode b){
  if((b.getLocation() ==null) && (ShippingMethod.AIR.equals(b.getShipmeth()))){
		
		Warehouse myWhse = new Warehouse();
		b.parcelroute.add(myWhse);
		b.setLocation(myWhse);
		
		Router air=new AirportRouter();
		
		Location originAir = air.findByZip(b.getFromZip());
		b.parcelroute.add(originAir);
		
		Location destinAir = air.findByZip(b.getToZip());
		b.parcelroute.add(destinAir);
